...
const short LIGHT_TRESHHOLD = 45;
const short STEP_SIZE = 1;
const short MAX_SPEED = 50;
const short MIN_DISTANCE = 20

bool hasSpottedObject = false;

void steer(int l, int r) {
	while (1) {
		if (l > motor[motorC]) {
			motor[motorLeft]--;
		} else {
		...
		if (l == motor[motorC] || r == motor[motorB]) {
			break;
		}
