...
#pragma config(Motor,  motorA,          motorEyes,     tmotorNXT, PIDControl, encoder)
...
void turnFaceDegrees(int degrees) {
	nMotorEncoder[motorEyes] = 0;
	nMotorEncoderTarget[motorEyes] = degrees;
	motor[motorEyes] = ((degrees > 0) ? 20 : -20);

	while(nMotorRunState[motorEyes] != runStateIdle) {
	  // Do not continue.
	}
	motor[motorEyes] = 0;
}
...
void turnRobotDegrees(int degrees) {
	...
	nMotorEncoderTarget[motorL] = degrees;
	motor[motorL] = ((degrees > 0) ? 20 : -20);
	motor[motorR] = ((degrees > 0) ? -20 : 20);
...
void stopState() {
	...
	if (command == "FIRE") {
		...
		motor[motorL] = 20;
		motor[motorR] = 20;
		displayBigTextLine(0, "%d", SensorValue[sensorSonar]);
		while (SensorValue[sensorSonar] < 25) {
			// Do nothing
		}
		turnFaceDegrees(90); // Look right
		...
		nMotorEncoder[motorL] = 0;
		nMotorEncoderTarget[motorL] = 360; // Robot lenth~
		motor[motorL] = 20;
		motor[motorR] = 20;
		while(nMotorRunState[motorL] != runStateIdle) {
		  // Do nothing
		...
		turnRobotDegrees(-90);
		// Move Eyes back
		// turnFaceDegrees(90); // Look left


		// Drive back to line
		motor[motorL] = 20;
		motor[motorR] = 20;
		while (SensorValue[sensorL] > 50 && SensorValue[sensorR] > 30) {
			// Do nothing
			displayBigTextLine(2, "%d, %d", SensorValue[sensorL], SensorValue[sensorR]);
		}
